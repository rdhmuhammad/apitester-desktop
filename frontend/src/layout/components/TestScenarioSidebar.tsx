        ...
        {filteredScenarios.length === 0 ? (
          ...
          return (
            <button key={scenario.id} type="button" onClick={() => handleSelectScenario(scenario.id)}
                    className={cn('group relative flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-sm hover:bg-sidebar-accent', isActive && 'bg-indigo-100 dark:bg-indigo-950/60')}>
              ...
              <span className="capitalize flex-1 truncate text-sidebar-foreground">{scenario.name}</span>
              ...
              {scenarios.length > 1 && (
                <span onClick={(event) => { event.stopPropagation(); handleDeleteScenario(scenario.id) }} role="button"
                      className="opacity-0 group-hover:opacity-100 p-1 text-slate-400 hover:text-rose-600 hover:bg-sidebar-accent rounded transition shrink-0" title="Delete Scenario">
